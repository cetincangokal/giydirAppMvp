...
class MessageSreen extends StatefulWidget {
  ...
  const MessageSreen({super.key, required this.myEmail, required this.sendEmail});
...
_firestore.collection('users').where('email',isEqualTo: widget.sendEmail).get().then((value) {
      setState(() {
        baslik = value.docs.first.data()['username'];
        debugPrint(baslik.toString());
      });
    });
  }
